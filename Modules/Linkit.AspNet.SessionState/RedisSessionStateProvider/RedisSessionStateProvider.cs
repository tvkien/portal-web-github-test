...
namespace Linkit.AspNet.SessionState
{
    public class RedisSessionStateProvider : SessionStateStoreProviderBase
    {
        ...
        internal static Dictionary<string, (ProviderConfiguration, RedisSharedConnection)> _configurationDict = new Dictionary<string, (ProviderConfiguration, RedisSharedConnection)>();
        ...
        public void RefreshConnection(System.Collections.Specialized.NameValueCollection config)
        {
            string connectionString = config["connectionString"];
            if (_configurationDict.TryGetValue(connectionString, out var value))
            {
                configuration = value.Item1;
                RedisConnectionWrapper.sharedConnection = value.Item2;
            }
            else
            {
                lock (configurationCreationLock)
                {
                    if (_configurationDict.TryGetValue(connectionString, out var value2))
                    {
                        configuration = value2.Item1;
                        RedisConnectionWrapper.sharedConnection = value2.Item2;
                    }
                    else
                    {
                        configuration = ProviderConfiguration.ProviderConfigurationForSessionState(config);
                        RedisConnectionWrapper.sharedConnection = new RedisSharedConnection(configuration);
                        _configurationDict[connectionString] = (configuration, RedisConnectionWrapper.sharedConnection);
                    }
                }
            }
        }
        ...
        public override SessionStateStoreData GetItem(HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actions)
        {
            ...
            var sessionData = GetItemFromSessionStore(false, context, id, out locked, out lockAge, out lockId, out actions);
            return sessionData;
        }
