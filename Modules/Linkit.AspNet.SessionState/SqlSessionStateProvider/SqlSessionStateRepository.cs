namespace Linkit.AspNet.SessionState
{
    ...
    class SqlSessionStateRepository : ISqlSessionStateRepository
    {
            ...
            int? retryInterval = DEFAULT_RETRY_INTERVAL, int? retryNum = DEFAULT_RETRY_NUM)
        {
            ...
            // Create SQL commands from templates once. (This constructor is 1-time-init enforced by SqlSessionStateProvider.Initialize)
        ...
        public void CreateSessionStateTable()
        {
            ...
            CreateSessionStateTableInitial();
        }
        private bool CreateSessionStateTableInitial()
        {
            ...
            using (var connection = new SqlConnection(_connectString))
            {
                ...
                try
                {
                    ...
                    _commandHelper.CreateSProcIfDoesNotExist(GetStateItemExclusiveSP, GetStateItemExclusiveSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(GetStateItemSP, GetStateItemSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(DeleteExpiredSessionsSP, DeleteExpiredSessionsSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(InsertUninitializedItemSP, InsertUninitializedItemSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(ReleaseItemExclusiveSP, ReleaseItemExclusiveSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(RemoveStateItemSP, RemoveStateItemSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(ResetItemTimeoutSP, ResetItemTimeoutSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(UpdateStateItemSP, UpdateStateItemSql, connection, transaction);
                    _commandHelper.CreateSProcIfDoesNotExist(InsertStateItemSP, InsertStateItemSql, connection, transaction);
                    transaction.Commit();
                }
                catch (Exception ex)
                {
                    try { transaction.Rollback(); } catch (Exception) { }
                    throw new HttpException(SqlResource.Cant_connect_sql_session_database, ex);
                }
            }
            return true;
        }
        public void DeleteExpiredSessions()
        {
            using (var connection = new SqlConnection(_connectString))
            {
                ...
                SqlSessionStateRepositoryUtil.SqlExecuteNonQueryWithRetry(connection, cmd, CanRetry);
            }
        }
        public SessionItem GetSessionStateItem(string id, bool exclusive)
        {
            var locked = false;
            ...
            using (var connection = new SqlConnection(_connectString))
            {
                using (var reader = SqlSessionStateRepositoryUtil.SqlExecuteReaderWithRetry(connection, cmd, CanRetry))
                {
                    if (reader.Read())
                    {
        ...
        public  void CreateOrUpdateSessionStateItem(bool newItem, string id, byte[] buf, int length, int timeout, int lockCookie, int orginalStreamLen)
        {
            SqlCommand cmd;
            if (!newItem)
            {
            ...
            using (var connection = new SqlConnection(_connectString))
            {
                SqlSessionStateRepositoryUtil.SqlExecuteNonQueryWithRetry(connection, cmd, CanRetry, newItem);
            }
        }
        public void ResetSessionItemTimeout(string id)
        {
        ...
        public void RemoveSessionItem(string id, object lockId)
        {
        ...
        public void ReleaseSessionItem(string id, object lockId)
        {
        ...
        public void CreateUninitializedSessionItem(string id, int length, byte[] buf, int timeout)
        {
            ...
            using (var connection = new SqlConnection(_connectString))
            {
                SqlSessionStateRepositoryUtil.SqlExecuteNonQueryWithRetry(connection, cmd, CanRetry, true);
            }
        }
        #endregion
        private bool CanRetry(RetryCheckParameter parameter)
        {
            ...
            return WaitToRetry(parameter, _retryIntervalMilSec);
        }
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static bool WaitToRetry(RetryCheckParameter parameter, int retryIntervalMilSec)
        {
