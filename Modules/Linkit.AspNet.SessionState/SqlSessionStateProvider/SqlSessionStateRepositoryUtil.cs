...
namespace Linkit.AspNet.SessionState
{
    ...
    class SqlSessionStateRepositoryUtil
    {
        ...
        public static int SqlExecuteNonQueryWithRetry(SqlConnection connection, SqlCommand sqlCmd,
        ...
        public static SqlDataReader SqlExecuteReaderWithRetry(SqlConnection connection, SqlCommand sqlCmd, Func<RetryCheckParameter, bool> canRetry,
            CommandBehavior cmdBehavior = CommandBehavior.Default)
        {
            ...
            while (true)
            {
                ...
                {
                    OpenConnection(connection);
                    return sqlCmd.ExecuteReader(cmdBehavior);
                }
                catch (SqlException e)
                {
                    retryParamenter.Exception = e;
        ...
        private static void OpenConnection(SqlConnection sqlConnection)
        {
            ...
            catch (SqlException e)
            {
                     ...
                     e.Number == SQL_LOGIN_FAILED_3))
                {
                    ...
                    throw new HttpException(string.Format(SqlResource.Login_failed_sql_session_database, user), e);
                }
            }
            catch (Exception e)
            {
                // just throw, we have a different Exception
                throw new HttpException(SqlResource.Cant_connect_sql_session_database, e);
            }
        }
