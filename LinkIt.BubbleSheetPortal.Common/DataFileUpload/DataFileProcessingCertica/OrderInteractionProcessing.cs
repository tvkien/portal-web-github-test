...
{
    public class OrderInteractionProcessing
    {
            ...
            DataFileUploaderResource resource)
        {
            try
            {
                resource.ProcessingStep.Append("Start Converting OrderInteraction.");
                var doc = DataFileProcessing.LoadXml(resource, parameter.ExtractedFoler);
                if (doc == null)
                {
                    return resource;
                }
                ...
                resource.Error = UpdateOrderInteraction(assessmentItemTag);
                 if (!string.IsNullOrEmpty(resource.Error))
                    return resource;    
        ...
        private static string UpdateOrderInteraction(XmlNode assessmentItemTag)
        {
            //get itemBody tag
