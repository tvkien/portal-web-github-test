...
{
    public class SchoolRosterValidator : AbstractValidator<School>
    {
        private readonly IRepository<School> repository;

        public SchoolRosterValidator(IRepository<School> repository)
        {
            this.repository = repository;

            RuleFor(x => x.DistrictId)
                .NotNull()
                .LessThanOrEqualTo(0).WithMessage("DistrictID > 0");
            ...
            RuleFor(x => x.Name)
                ...
                .NotEmpty()
                .Length(0, 100)
                .Must(NameMustBeUniqueToDistrict())
            ...
            RuleFor(x => x.Code)
                ...
                .WithMessage("School Code already exists.");
        ...
        private Func<School, string, bool> NameMustBeUniqueToDistrict()
        {
            return (model, schoolName) => repository.Select().FirstOrDefault(x =>
                model.Id != x.Id && x.DistrictId.Equals(model.DistrictId) && x.Name.Equals(schoolName)).IsNull();
        }
