...
; (function ($, window, document, undefined) {
  ...
  ComboTree.prototype.createSourceItemHTML = function (sourceItem) {
    ...
    if (this.options.isMultiple)
      itemHtml += '<span data-id="' + sourceItem.id + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + ' selectable-' + isSelectable + '">' + (!selectableLastNode && isSelectable ? '<input type="checkbox" />' : '') + sourceItem.title + '</span>';
    else
      itemHtml += '<span data-id="' + sourceItem.id + '" data-text="' + sourceItem.title + '" data-selectable="' + isSelectable + '" class="comboTreeItemTitle ' + selectableClass + ' selectable-' + isSelectable + '">' + sourceItem.title + '</span>';
  ...
  ComboTree.prototype.bindings = function () {
    ...
    this._elemInput.on('keydown', function (e) {
      e.stopPropagation();
      if (e.keyCode != 13 && !_this._elemDropDownContainer.is(":visible"))
        _this.toggleDropDown();
      switch (e.keyCode) {
        case 9:
          _this.closeDropDownMenu(); break;
    ...
    ComboTree.prototype.filterDropDownMenu = function () {
      ...
      if (searchText != "") {
        ...
        list = this._elemItems.filter(function (index, item) {
          return (item.textContent || item.innerHTML).toLowerCase().indexOf(searchText.toLowerCase()) != -1;
        }).each(function (i, elem) {
          $(this.children).show()

          results.push(this.children);
          $(this).siblings("span.comboTreeParentPlus").show();
        });

        var findResult = results.filter(function (item, index) {
          if (item[0].dataset.selectable === 'true') {
            return item;
          }
        });
        if (findResult.length === 1) {
          this.dropDownMenuHover($(findResult[0]));
        }

        var controlIdNotFound = this.comboTreeId + "_notfound";
        if (list.length === 0) {
          if (this._elemDropDownContainer.find("#" + controlIdNotFound).length === 0) {
            this._elemDropDownContainer.append('<ul id="' + controlIdNotFound + '"><li><span class="comboTreeItemTitle selectable selectable-false">No results found</span></li></ul>');
          }
        } else {
          if (this._elemDropDownContainer.find("#" + controlIdNotFound).length) {
            this._elemDropDownContainer.find("#" + controlIdNotFound).remove();
          }
        }
      }
      else {
