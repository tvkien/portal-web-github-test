...
(function($)
{
	...
	function getTipPosition(element, tip, settings, animStart)
	{
      ...
      default:
        var positionLeft = Math.round(offset.left + (element.outerWidth() / 2) - (tip.outerWidth() / 2))
        if (positionLeft < 0) {
          positionLeft = 0;
          tip.find('.arrow').css('left', 'calc(50% + ' + positionLeft + 'px)');
        }
        if ((positionLeft + tip.outerWidth()) >= $("html").width()) {
          newPositionLeft = $("html").width() - tip.outerWidth();
          deltaLeft = positionLeft - newPositionLeft;
          positionLeft = newPositionLeft;
          tip.find('.arrow').css('left', 'calc(50% + ' + deltaLeft + 'px)');
        }
				return {
