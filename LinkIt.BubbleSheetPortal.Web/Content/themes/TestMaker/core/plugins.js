...
CKEDITOR.plugins.setLang = function (pluginName, languageCode, languageEntries) {
    ...
    if (plugin != undefined)
    {
        var pluginLangEntries = plugin.langEntries || (plugin.langEntries = {}),
...
CKEDITOR.plugins.getImgByVersion = function(pluginName, relativePath) {
	if (IS_V2) {
		var path = relativePath;
		if (CKEDITOR.plugins.get(pluginName)) {
			path = CKEDITOR.plugins.get(pluginName).path + relativePath;
			path = path.replace(CKEDITOR.basePath, '');
			if (path.startsWith('plugins/')) {
				path = path.replace('plugins/', '');
			}
		} else {
			path = pluginName + '/' + relativePath;
		}
		return CKEDITOR.basePath + 'v2/' + path.split('/')
			.join('_')
			.replace('.png', '.svg') + window.CKE_VERSION;
	} else {
		return CKEDITOR.basePath + 'plugins/' + pluginName + '/' + relativePath;
	}
}
