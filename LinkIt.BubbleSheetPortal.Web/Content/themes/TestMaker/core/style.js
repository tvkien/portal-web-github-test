...
( function() {
    ...
    //no change style
	CKEDITOR.editor.prototype.noChange = function (style) {
	    return;
	};
	...
	function applyBlockStyle( range ) {
		...
		while ( ( block = iterator.getNextParagraph() ) ) {
		    if (!block.isReadOnly() && checkIfAllowedByIterator(iterator, this)) {
		        if ($(block.$).html() != '&nbsp;' || $(block.$).find('span').html() != null) {
		...
		while ( ( block = iterator.getNextParagraph() ) ) {
			if ( this.checkElementRemovable( block ) ) {
				...
				if ( block.is( 'pre' ) ) {
				    ...
				    //remove br last before insert into the DOM
				    if ($(block.$).find('br').length >= 2) {
				        $(block.$).find('br').last().remove();
				        noRemoveFirstBr = true;
				    }
			    
				    removeFromElement.call(this, block);
			        noRemoveFirstBr = false; //reset hanle box text
			}
		}
	...
	function replaceBlock( block, newBlock ) {
	    ...
	    //remove sign &#8203; into boxtext editor
		var nodeListItemBoxText = newBlock.getChildren();
		var lastNote = nodeListItemBoxText.getItem(nodeListItemBoxText.$.length - 1);
	    if (lastNote.type === 3) {
		    lastNote.$.remove();
		}

	    //remove &nbsp; when create box text
		var htmlString = newBlock.getHtml().replace(/&nbsp;/g, '');
		$(newBlock.$).html(htmlString);
	    //append again br into DOM and make sure user can focus behind boxes
		$(newBlock.$).after('&nbsp;<br>');
	...
	function removeFromElement( element, keepDataAttrs ) {
		...
		if ($(element.$).find('br').length === 1 && !noRemoveFirstBr) {
		...
		if ( !element.hasAttributes() || forceRemove ) {
			if ( CKEDITOR.dtd.$block[ element.getName() ] ) {
				...
				if (next && (next.type == CKEDITOR.NODE_TEXT || !next.isBlockBoundary({ br: 1 }))) {
				    if (!noAppendBrEditor) { //no append br when no select box text
				        element.append('br');
				    }
				}
