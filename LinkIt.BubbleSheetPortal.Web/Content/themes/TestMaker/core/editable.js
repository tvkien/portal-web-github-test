	...
	var insert = (function() {
		...
		function prepareRangeToDataInsertion( that ) {
			...
			if ( ( node = path.contains( isInline, false, 1 ) ) ) {
				...
				// LNKT-56104 dont create dummy boxedText Element
				var splitElement = range.splitElement( node );
				if (path.lastElement.$.classList.contains('boxedText')) {
					$(path.lastElement.$).find('br').last().remove();
					splitElement.remove();
					!$(path.lastElement.$).prop('innerHTML') && path.lastElement.$.remove();
				}
			}
