...
(function() {
	...
	function insertInlineChoiceRow(selection) {
	    var cells = getSelectedCells(selection),
			firstCell = cells[0],
			table = firstCell.getAscendant('table'),
	        startRow = cells[0].getParent(),
	    ...
	    refreshResponseId(); //This make sure the responseID of items are same on item editor

        //Insert inline choice to table
	    for (i = 0; i < table.$.rows.length; i++) {
	        if (i == startRowIndex)
	        {
	            for (c = 0; c < table.$.rows[i].cells.length; c++) {
	                ...
	                table.$.rows[i].cells[c].innerHTML = '&nbsp;<span id="' + responseID + '" class="inlineChoiceInteraction" contenteditable="false"><img class="cke_reset cke_widget_mask inlineChoiceInteractionMark" src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" /></span>&nbsp;';
	...
	function insertTextEntryRow(selection) {
	    var cells = getSelectedCells(selection),
			firstCell = cells[0],
			table = firstCell.getAscendant('table'),
            startRow = cells[0].getParent(),
			startRowIndex = startRow.$.rowIndex;
	        //startColumnIndex = firstCell.getIndex();
	    ...
	    for (i = 0; i < table.$.rows.length; i++) {
	        if (i == startRowIndex) {
	            for (c = 0; c < table.$.rows[i].cells.length; c++) {
	                //Create response identify and make sure it doesn't conflict with current.
	                var responseID = createResponseId();
	                table.$.rows[i].cells[c].innerHTML = '&nbsp;<span id="' + responseID + '" class="textEntryInteraction" contenteditable="false"><img class="cke_reset cke_widget_mask textEntryInteractionMark" src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" /></span>&nbsp;';
	                iResult.push({ correctResponse: [{ value: "Answer Correct 1", pointsValue: "1" }], expectedLength: "8", responseDeclaration: { baseType: "string", cardinality: "single", method: "default", pointsValue: "1", spelling: false, spellingDeduction: "1", type: "string", caseSensitive: "false" }, responseIdentifier: responseID, type: "textEntryInteraction" });
	            }
	        }
	    }
	}

	function insertInlineChoiceColumn(selection) {
	    var cells = getSelectedCells(selection),
			firstCell = cells[0],
			table = firstCell.getAscendant('table'),
	        startColumnIndex = firstCell.getIndex();
	...
	function insertTextEntryColumn(selection) {
	    var cells = getSelectedCells(selection),
			firstCell = cells[0],
			table = firstCell.getAscendant('table'),
	        startColumnIndex = firstCell.getIndex();
	...
	CKEDITOR.plugins.tabletools = {
		...
		init: function( editor ) {
			...
			addCmd('rowInsertInlineChoice', createDef({
			    requiredContent: 'table',
			    exec: function (editor) {
			        var selection = editor.getSelection();
			        insertInlineChoiceRow(selection);
			    }
			}));

			addCmd('rowInsertTextEntry', createDef({
			    requiredContent: 'table',
			    exec: function (editor) {
			        var selection = editor.getSelection();
			        insertTextEntryRow(selection);
			    }
			}));
			...
			addCmd('columnInsertInlineChoice', createDef({
			    requiredContent: 'table',
			    exec: function (editor) {
			        var selection = editor.getSelection();
			        insertInlineChoiceColumn(selection);
			    }
			}));

			addCmd('columnInsertTextEntry', createDef({
			    requiredContent: 'table',
			    exec: function (editor) {
			        var selection = editor.getSelection();
			        insertTextEntryColumn(selection);
			    }
			}));
				...
				editor.addMenuItems({
					tablecell: {
						...
						getItems: function() {
                            ...
                            /*if (showTableAddItem == true)
                            {
                                returnMenu.tablecell_inlinechoice = CKEDITOR.TRISTATE_OFF;
                                returnMenu.tablecell_textentry = CKEDITOR.TRISTATE_OFF;
                            }*/
					...
					tablerow: {
						...
						getItems: function() {
							...
							if (showTableAddItem == true) {
							    returnMenu.tablerow_inlinechoice = CKEDITOR.TRISTATE_OFF;
							    returnMenu.tablerow_textentry = CKEDITOR.TRISTATE_OFF;
							}
					...
					tablerow_inlinechoice: {
					    label: labelCkeditor.inlineChoice,
					    group: 'tablerow',
					    command: 'rowInsertInlineChoice',
					    order: 1
					},

					tablerow_textentry: {
					    label: labelCkeditor.textEntry,
					    group: 'tablerow',
					    command: 'rowInsertTextEntry',
					    order: 2
					},
					...
					tablecolumn: {
						...
						getItems: function () {
						    ...
						    if (showTableAddItem == true) {
						        returnMenu.tablecolumn_inlinechoice = CKEDITOR.TRISTATE_OFF;
						        returnMenu.tablecolumn_textentry = CKEDITOR.TRISTATE_OFF;
						    }
					...
					tablecolumn_inlinechoice: {
					    label: labelCkeditor.inlineChoice,
					    group: 'tablecolumn',
					    command: 'columnInsertInlineChoice',
					    order: 1
					},

					tablecolumn_textentry: {
					    label: labelCkeditor.textEntry,
					    group: 'tablecolumn',
					    command: 'columnInsertTextEntry',
					    order: 2
					},
