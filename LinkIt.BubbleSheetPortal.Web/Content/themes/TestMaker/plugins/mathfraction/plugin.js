...
( function() {
	// Change TeX format to MathML format
	//var cdn = 'http:\/\/cdn.mathjax.org\/mathjax\/2.2-latest\/MathJax.js?config=TeX-AMS_HTML';
    var cdn = window.location.protocol + '\/\/s3-us-west-2.amazonaws.com\/linkit-utility\/MathJax\/MathJax.js?config=MML_HTMLorMML';

	CKEDITOR.plugins.add( 'mathfraction', {
		...
		init: function( editor ) {
			...
			editor.widgets.add( 'mathfraction', {
				inline: true,
				dialog: 'mathfraction',
				button: editor.lang.mathfraction.button,
				mask: true,
				allowedContent: 'span(!' + cls + ')',
				pathName: editor.lang.mathfraction.pathName,
				...
				defaults: {
				    math: '<math class="simpleFraction" xmlns="http://www.w3.org/1998/Math/MathML"><mstyle displaystyle="true"><mrow class="MJX-TeXAtom-ORD"><mfrac><mn>1</mn><mn>x</mn></mfrac></mrow></mstyle></math>'
				},
				...
				init: function() {
					...
					if ( !iframe || iframe.type != CKEDITOR.NODE_ELEMENT || !iframe.is( 'iframe' ) ) {
						...
						iframe.setAttributes( {
							...
							src: CKEDITOR.plugins.mathfraction.fixSrc
						} );
					...
					this.once( 'ready', function() {
						...
						if ( CKEDITOR.env.ie )
							iframe.setAttribute( 'src', CKEDITOR.plugins.mathfraction.fixSrc );

						this.frameWrapper = new CKEDITOR.plugins.mathfraction.frameWrapper( iframe, editor );
			...
			// Add dialog.
			CKEDITOR.dialog.add( 'mathfraction', this.path + 'dialogs/mathjax.js' );
	 ...
	 */
	CKEDITOR.plugins.mathfraction.fixSrc =
	 ...
	 * @member CKEDITOR.plugins.mathjax
	 */
	CKEDITOR.plugins.mathfraction.loadingIcon = CKEDITOR.plugins.get( 'mathfraction' ).path + 'images/loader.gif';
	 ...
	 */
	CKEDITOR.plugins.mathfraction.copyStyles = function( from, to ) {
	 ...
	 */
	CKEDITOR.plugins.mathfraction.trim = function( value ) {
	...
	if ( !( CKEDITOR.env.ie && CKEDITOR.env.version == 8 ) ) {
		CKEDITOR.plugins.mathfraction.frameWrapper = function( iFrame, editor ) {
			...
			var buffer, preview, value, newValue,
				...
				updateDoneHandler = CKEDITOR.tools.addFunction( function() {
					CKEDITOR.plugins.mathfraction.copyStyles( iFrame, preview );
			...
			function load() {
				...
				doc.write( '<!DOCTYPE html>' +
							...
							'<head>' +
								...
								'<style>.MathJax > nobr > .math > :empty { border-left-color: transparent !important; }</style>' +
							'</head>' +
			...
			function update() {
				...
				preview.setHtml( '<img src=' + CKEDITOR.plugins.mathfraction.loadingIcon + ' alt=' + editor.lang.mathfraction.loading + '>' );
				...
				// Run MathJax.
				//value = CKEDITOR.plugins.mathjax.trim(value);
		...
		CKEDITOR.plugins.mathfraction.frameWrapper = function( iFrame, editor ) {
			...
			return {
				setValue: function( value ) {
					...
					CKEDITOR.plugins.mathfraction.copyStyles( iFrame, tex );
