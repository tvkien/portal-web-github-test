...
(function() {
	...
	template +=
		...
		' onfocus="return CKEDITOR.tools.callFunction({focusFn},event);" ' +
			...
			'<span id="{id}_text" {combo_text_style} class="cke_combo_text cke_combo_inlinelabel">{label}</span>' +
	...
	( CKEDITOR.env.hc ? '&#9660;' : CKEDITOR.env.air ? '&nbsp;' : '' ) +
		...
		'</a>' +
		'</span>';
	template = template.replace('{combo_text_style}', IS_V2 ? 'style="color: var(--navyColor); width: auto;"' : '');
			...
			createPanel: function( editor ) {
				...
				panel.onShow = function() {
					...
					editor.once( 'panelShow', function() {
					    ...
					    if (isShowPanelFontSize) {
					        if (!(navigator.userAgent.indexOf('AppleWebKit') > -1)) {
					            var valFontSize = $('.editorArea .cke_combo__fontsize').find('a .cke_combo_text').text();
				...
				panel.onHide = function( preventOnClose ) {
                    ...
                    //reset default show panel font size
