...
( function() {
	...
	CKEDITOR.dialog = function( editor, dialogName ) {
		...
		if (( buttonsOrder == 'rtl' && dir == 'ltr' ) || ( buttonsOrder == 'ltr' && dir == 'rtl' ) )
			...
			iterContents( function( item ) {
				if ( item.isChanged() ) {
					if ( !editor.config.dialog_noConfirmCancel) {
						evt.data.hide = false;
						if (!that._confirmOpenning) {
							that._confirmOpenning = true;
							customConfirm( editor.lang.common.confirmCancel ).then(function(sure) {
								that._confirmOpenning = false;
								if (sure) {
									that.hide();
								}
							})
		...
		this.on( 'show', function() {
			...
			if (IS_V2) {
				...
				dialog.setStyle('left', (document.documentElement.clientWidth - rect.width) / 2 + 'px');
				dialog.setStyle('top', Math.max(20, document.documentElement.scrollTop + (window.innerHeight - rect.height - 70) / 2) + 'px');
			}
	...
	CKEDITOR.dialog.prototype = {
      ...
      // Set z-index.
      if ($.ui && $.ui.dialog && $.ui.dialog.currentZ() !== -Infinity) {
        CKEDITOR.dialog._.currentZIndex = $.ui.dialog.currentZ();
        $.ui.dialog.maxZ = CKEDITOR.dialog._.currentZIndex + 10;
      }
			else if ( CKEDITOR.dialog._.currentZIndex === null )
...
CKEDITOR.plugins.add( 'dialog', {
	requires: 'dialogui',
	init: function( editor ) {
		editor.on( 'doubleclick', function( evt ) {//check here for double click
			if ( evt.data.dialog )
