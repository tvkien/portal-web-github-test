	...
	CKEDITOR.plugins.add( 'mathjax', {
		...
		init: function( editor ) {
			...
			editor.widgets.add( 'mathjax', {
				...
				title: editor.lang.mathjax.title,
				...
				defaults: {
					math: '<math mode="display" xmlns="http://www.w3.org/1998/Math/MathML"><mrow><mn>1</mn><mo>+</mo><mi>y</mi></mrow></math>'
				},
				...
				init: function() {
					...
					this.once( 'ready', function() {
						...
						var data = this.data.math.trim();
						var firstMath = data.substring(0, 5);
						var lasttMath = data.substr(-7);

						this.frameWrapper = new CKEDITOR.plugins.mathjax.frameWrapper(iframe, editor);
                        
						if (firstMath == '<math' && lasttMath == '</math>') {
						    this.frameWrapper.setValue(data);
						}
						else {
						    alert('Unexpected text node');
						}
