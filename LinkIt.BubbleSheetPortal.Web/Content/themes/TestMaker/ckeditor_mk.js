    ...
    if (ev.editor.name == ckID) {
      if (isAddnew) {
        ...
        if (iSchemeID == "1") {
          ...
          var ckEditorInlineChoice = '<span contenteditable="false" class="placeholder" style="position: absolute;z-index: 99999;"> ';
          ckEditorInlineChoice += ' Enter Question...</span><br/>&nbsp; ';
          ckEditorInlineChoice += ' <span id = "RESPONSE_1" style = "max-width: 250px;" class="inlineChoiceInteraction" contenteditable = "false" > ';
          ckEditorInlineChoice += ' <img style="display: none;padding-left: 3px; padding-top: 0.5px;" alt="Guidance" class="imageupload bntGuidance" src="../../Content/themes/TestMaker/images/guidance_checked_small.png" title="Guidance"> ';
          ckEditorInlineChoice += ' <img class="cke_reset cke_widget_mask inlineChoiceInteractionMark" src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" />';
          ckEditorInlineChoice += ' </span><span>&nbsp;</span><br><br>';

          CKEDITOR.instances[ckID].setData(ckEditorInlineChoice);
          iResult = [{
            correctResponse: "A",
            inlineChoice: [{
              arrMessageGuidance: [],
...
function xmlExport(isTextToSpeech) {
  ...
  $(tempData).find('table').each(function () {
    var table = this;
    ...
    if (tableAudioId) {
      ...
      if (hasAudio == false) {
        ...
        var newTable = table.outerHTML;
        tempData = tempData.replace(oldTable, newTable);
      }
    }
  ...
  $.each(inlineChoiceInteraction, function () {
    ...
    for (var i = 0; i < iResult.length; i++) {
      if (iResult[i].responseIdentifier == currentResId && iResult[i].type == "inlineChoiceInteraction") {
        ...
        var buildInlineChoiceInteraction = '<inlineChoiceInteraction expectedWidth="' + expectedWidth + '" visibleDimension="' + visibleDimension + '" ';
        buildInlineChoiceInteraction += ' responseIdentifier = "' + iResult[i].responseIdentifier + '" shuffle = "' + iResult[i].shuffle + '" >';
  ...
  $.each(tableHotSpot, function () {
    ...
    for (var i = 0, iResultLen = iResult.length; i < iResultLen; i++) {
      if (iResult[i].responseIdentifier === currentResId && iResult[i].type === "tableHotSpot") {
        ...
        var tagTds = $(buildTableHotSpot).find('tr td');
        var tagHotSpots = tagTds.find('span[typehotspot]');

        $.each(tagHotSpots, function () {
          var oldHotSpot = $(this).prop("outerHTML");
          var typehotspot = '';
          var classhotspot = '';
          var newHotSpot = '';
          ...
          if ($(this).attr("typehotspot") === 'circle') {
            typehotspot = 'circle';
          ...
          newHotSpot = '<tableitem pointvalue="' + pointValue + '" typehotspot="' + typehotspot + '" class="' + classhotspot + '" identifier="' + idHotSpot + '" contenteditable="false" ischecked="' + ischecked + '">&nbsp;<label class="tooltipHotSpot" style="' + objcss + '">' + idTooltip + '</label></tableitem>';
          buildTableHotSpot = buildTableHotSpot.replace(oldHotSpot, newHotSpot);
        });

        tempData = tempData.replace(currentHtml, buildTableHotSpot);
        break;
      }
    }
  });
        ...
        for (var k = 0; k < iResult[i].source.length; k++) {
          if ($partialSourceObj.attr("srcidentifier") == iResult[i].source[k].srcIdentifier) {
            ...
            if (iResult[i].source[k].type === 'text') {
              ...
              tempData = tempData.replace($partialSourceObj.prop('outerHTML'), '<sourceObject partialID="Partial_1" style="width: ' + wTemp + '; height: ' + $partialSourceObj.css('height') + ';" srcIdentifier="' + $partialSourceObj.attr("srcidentifier") + '" type="' + iResult[i].source[k].type + '" data-limit="' + iResult[i].source[k].limit + '">' + iResult[i].source[k].value + '</sourceObject>');
            } else {
              ...
              tempData = tempData.replace($partialSourceObj.prop('outerHTML'), '<sourceObject partialID="Partial_1" srcIdentifier="' + $partialSourceObj.attr("srcidentifier") + '" type="' + iResult[i].source[k].type + '"' + strFloatSource + ' data-limit="' + iResult[i].source[k].limit + '">' + iResult[i].source[k].value + '</sourceObject>');
            }
            break;
          }
        }
        ...
        if ($partialDesObj.attr("type") == "text") {
          ...
          $partialDesObj.find(".hotSpot").each(function (index, hotspot) {
            ...
            if (gridcell !== undefined && gridcell === 'true') {
              hotSpot += '<destinationItem destIdentifier="' + hotspotIdentifier + '" left="' + hotspotLeft + '" top="' + hotspotTop + '" width="' + hotspotWidth + '" height="' + hotspotHeight + '" order="' + order + '" gridcell="true" numberDroppable="' + hotspotNumberDroppable + '"></destinationItem>';
            } else {
              hotSpot += '<destinationItem destIdentifier="' + hotspotIdentifier + '" left="' + hotspotLeft + '" top="' + hotspotTop + '" width="' + hotspotWidth + '" height="' + hotspotHeight + '" order="' + order + '" numberDroppable="' + hotspotNumberDroppable + '"></destinationItem>';
            }
          });
...
function xmlImport(xmlContent, schemeID, pointsValueAlgorithmic) {
      ...
      if (resId == $(this).attr("identifier")) {
        ...
        if (typeof $inlinechoice.attr('data-top') !== 'undefined' ||
          ...
          strInlineChoiceReplace += '</span>\u200B'
            ...
            strInlineChoiceReplace += '<span style="margin-right: 0.5px; max-width:' + _expectedWidth + 'px" ';
          strInlineChoiceReplace += ' id="' + resId + '" title="' + resId + '" ';
          strInlineChoiceReplace += ' class="inlineChoiceInteraction" contenteditable="false">';
          strInlineChoiceReplace += strInlineChoiceMask;
          strInlineChoiceReplace += '</span>\u200B'
        }
      ...
      $(xmlContent).find("responseDeclaration").each(function () {
        ...
        if (resId == $(this).attr("identifier")) {
          ...
          $(currentChoice).find("simpleChoice").each(function (index) {
            ...
            htmlMulChoice += '<img alt="Play audio" class="bntPlay" src="../../Content/themes/TestMaker/images/small_audio_play.png" title="Play audio">';
            htmlMulChoice += '<img alt="Stop audio" class="bntStop" src="../../Content/themes/TestMaker/plugins/multiplechoice/images/small_audio_stop.png" title="Stop audio">';
            htmlMulChoice += '<span class="audioRef">' + audioLink + '</span></div>';
            htmlMulChoice += "<div style='display: none;' class='" + hastypeMessageGuidance + "'>";
            htmlMulChoice += '    <img alt="Guidance" class="imageupload bntGuidance" src="../../Content/themes/TestMaker/images/guidance_checked.png" title="Guidance">';
            htmlMulChoice += "</div>";
          ...
          newChoiceInteraction = '<div textTrueFalse="' + textTrueFalse + '" class="multipleChoice" id="' + resId + '" title="' + resId + '" contenteditable="false"><button class="single-click" id="single-click">Click here to edit answer choices</button><img class="cke_reset cke_widget_mask multipleChoiceMark" src="data:image/gif;base64,R0lGODlhAQABAPABAP///wAAACH5BAEKAAAALAAAAAABAAEAAAICRAEAOw%3D%3D" />' + htmlMulChoice + ' </div>';
  ...
  if ($(xmlContent).find(".linkit-table").hasClass("tableHotspotInteraction")) {
...
function loadEditItem(schemeId, isNew) {
  ...
  if (schemeId === '21') {
    ...
    var noDuplicateHTML = '<div class="noDuplicateHTML"><input type="checkbox" id="noDuplicate" /> <label for="noDuplicate">No Duplicate Answers</label</div><div class="clear10"></div>';
  ...
  var ckConfig = {
    contentsCss: window.location.protocol + '//' + window.location.hostname + '/Content/themes/TestMaker/contents.css',
