...
$('#manualEntryButton').click(function() {
    if (($('#manualEntryButton')[0].innerHTML == 'Keyboard Entry') ||
    ($('#manualEntryButton')[0].innerHTML == 'Perform Manual Entry')) {
        $.getScript("../../Scripts/soundmanager2-nodebug-jsmin.js", function() {
            ...
            soundManager.onload = function() {
                $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Loading Sounds ...';
        ...
        var diff = new Date() - focusoutTime; // already focusout over 250ms
        if ($('#manualEntryButton')[0].innerHTML == 'Resume Manual Entry' && diff > 250) {
            startManualEntry();
        } else {
            //$('#manEntryGuide').hide().css({ "display": "none", "visibility": "hidden" }); // Task show/hide instruction
            $('#manualEntryButton')[0].innerHTML = 'Keyboard Entry';
        }
    }
});

$('#aSelection').click(function () {
    for (cnt = 0; cnt < answerList.length; cnt++) {

        var cqObj = answerList[cnt];

        try { answerList[cnt]['A'][0].checked = true; }
        catch (err) { }
...
function focusOnQuestion(questionNumber, isSelectAllAClickValue) {
    ...
    if (cqObj == null) {
        ...
        if ($('#manualEntryButton')[0] != null) {
            if ($('#manualEntryButton')[0].innerHTML != 'Keyboard Entry'
                    && $('#manualEntryButton')[0].innerHTML != 'Resume Manual Entry') {
                $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Started Manual Entry';

                //$('#manEntryGuide').show().css({ "display": "block", "visibility": "visible" }); // Task show/hide instruction
                //$('#manEntryGuide').css('opacity', '1');
            }
        }
        return;
    }
...
function startManualEntry() {
    ...
    if ($('#manualEntryButton')[0].innerHTML == 'Loading Sounds ...') {
        $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Started Manual Entry';
    ...
    if (FocusingOnQuestion) {
        if ($('#manualEntryButton')[0] != null) {
            $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Resume Manual Entry';
        }

        //$('#manEntryGuide').hide().css({ "display": "none", "visibility": "hidden" });

        //$('#manEntryGuide').css('opacity', '0.5'); // Task show/hide instruction
...
$('.radioButtonAnswer').focusin(function () {
    if ($('#manualEntryButton')[0] == null) {
        return;
    }
    if ($('#manualEntryButton')[0].innerHTML == 'Keyboard Entry') {
    ...
    if (isRadioButtonClicked == false && isSelectAllAClicked == false) {
        if ($('#manualEntryButton')[0] != null) {
            $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Started Manual Entry';
        }
...
$('.radioButtonAnswer').focusout(function () {
    //debugger;
    if ($('#manualEntryButton')[0] != null && $('#manualEntryButton')[0].innerHTML == 'Started Manual Entry') {
        $('#manualEntryButton')[0].innerHTML = $('#manualEntryButton')[0].innerHTML = 'Resume Manual Entry'; // TODO:
...
$('.radioButtonAnswer').keypress(function (event) {
...
$(document).keydown(function (event) {    
    ...
    if (event.keyCode == 32 && $('#manualEntryButton')[0] != null && $('#manualEntryButton')[0].innerHTML == 'Started Manual Entry') {
        event.preventDefault();
    }
});
...
function viewBubbleSheetDetail(isApplyGradingShortcut, answers) {
    ...
    var studentId = $($('#dataTable .student')[$('#selectedStudentIndex').val()]).attr('StudentId');
...
$('#btnDisplayInstructions').click(function () {
    if ($('#manEntryGuide').css('display') == 'none') {
        ...
        $('#btnDisplayInstructions').text('Hide Instructions');
    } else {
        $('#manEntryGuide').hide().css({ "display": "none", "visibility": "hidden" });
        $('#btnDisplayInstructions').text('Show Instructions');
    }        
});
//############################ Support select student by keyboard after submmiting #########################################