(function ($) {
    $.widget('jquery.TableHotspotAlgorithmic', {
        options: {
            TableHotspotAlgorithmicUtil: null,
        ...
        Display: function (question) {
            ...
            // Update answer text
            if (!options.TableHotspotAlgorithmicUtil.IsNullOrEmpty(question.Answer())) {
                answerText = question.Answer().AnswerText();
            }

            // Mapping answer
            if (!options.TableHotspotAlgorithmicUtil.IsNullOrEmpty(answerText)) {
                mappingsAnswer.push(answerText);
            }

            // Update correct answer
            if (!options.TableHotspotAlgorithmicUtil.IsNullOrEmpty(question.CorrectAnswer())) {
        ...
        ShowAllCorrectAnswers: function (self, question) {
            ...
            ko.utils.arrayForEach(question.AlgorithmicCorrectAnswers(), function (item) {
                ...
                if (typeof item.Amount === 'function' && item.Amount() > 0) {
                    ...
                    correctHtml = that.GetContentTableHotspot(self, $(question.ItemBody()), correctAnswer);
                    tree = $('<div/>');
