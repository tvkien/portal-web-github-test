(function ($) {
    $.widget('jquery.TextEntryAlgorithmic', {
        options: {
            TextEntryAlgorithmicUtil: null,
        ...
        Display: function (question) {
            ...
            var selectedAnswer = '';
            if (!options.TextEntryAlgorithmicUtil.IsNullOrEmpty(question.Answer())) {
                responseProcessingTypeID = question.Answer().ResponseProcessingTypeID();
                selectedAnswer = question.Answer().AnswerText();
            }

            var textEntryMark = $('');
            if (!options.TextEntryAlgorithmicUtil.IsNullOrEmpty(self.QTIOnlineTestSessionID()) && (self.IsComplete() || self.IsPendingReview()) && responseProcessingTypeID != '2') {
        ...
        ShowAllCorrectAnswers: function (self, question) {
            ...
            ko.utils.arrayForEach(question.AlgorithmicCorrectAnswers(), function (item) {
                ...
                var tree = that.GetContentTextEntry(self, question, correctAnswer);
        ...
        GetContentTextEntry: function (self, question, correctAnswerTextAbc) {
            var tree = $('<div></div>');
            ...
            tree.find('textEntryInteraction').replaceWith(function () {
                ...
                var correctAnswer = $('<span idresponse=' + responseIdentifier + ' class="correct-answer-teacher-review"></span>');
                correctAnswer.css('color', 'red').text('Correct Answer: ');

                if (correctAnswerTextAbc.substring(0, 1) === '=') {
                    correctAnswerTextAbc = correctAnswerTextAbc.substring(1);                    
                }

                if (correctAnswerTextAbc === '*') {
                    correctAnswerTextAbc = 'Anything else';
                }

                correctAnswer.append(correctAnswerTextAbc);

                return $(newTextEntry.outerHTML() + correctAnswer.outerHTML());
            });
