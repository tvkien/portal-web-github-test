      ...
      decodeHTMl: function(string) {
        ...
        Display: function (question) {
            ...
            $.each(tagsUl, function (idx, tagul) {
                ...
                var wtagUl = $(tagul).outerWidth() + 52;
        ...
        _renderSimpleChoiceResult: function (questionItemBody, testOnlineSessionAnswerList, question, self) {
            ...
            $(choiceInteraction).find('simpleChoice').each(function () {
                ...
                simpleChoice.find('div.answer').each(function () {
                    ...
                    divIdentifier.append('<div stylename="answer">' + html + '</div>');
                  ...
                  if (isStudentChoose) {
                      ...
                      divIdentifier.append('<span class="jsIsUserAnswer"><i class="fa-solid fa-user me-2"></i> Student’s answer</span>');
                  }
                });
            ...
            var isAutoOverridedClass = (overridden && isReviewed) ? 'is-auto-overrided' : '';
            answerContainer = '<div class="clearfix"></div><div responseidentifier="' + $(choiceInteraction).attr('responseidentifier') + '" class="box-answer ' + isAutoOverridedClass + '" onclick="' + onclick + '" >' + answerContainer + '<div class="clearfix"></div></div>';
        ...
        _renderInlineChoiceResult: function (self, questionItemBody, testOnlineSessionAnswerList, question) {
          ...
          $(questionItemBody).find('inlinechoice').each(function () {
                ...
                var originInlineChoice = originChoiceInteraction.find('inlineChoice[identifier="' + identifier + '"]');
                var option1 = this.outerHTML.replaceAll('inlinechoice', 'li');
                if ($(this).find('div[typemessage]').length || originInlineChoice.find('div[typemessage]').length) {
                    var arrGuidances = $(this).find('div[typemessage]');
                    if (arrGuidances.length === 0) {
                      arrGuidances = originInlineChoice.find('div[typemessage]');
                    }
                    var valItemOption = '';
                ...
                ko.utils.arrayForEach(testOnlineSessionAnswerList, function (testOnlineSessionAnswer) {
                    if (question.VirtualQuestionID() === testOnlineSessionAnswer.VirtualQuestionID()) {
                        ...
                        if (htmlGuidance !== undefined && htmlGuidance !== '') {
                            var iconString = '<img responseidentifier="' + responseidentifier + '" alt="Guidance" style="margin-left: 5px; width: 1.5em; height: 1.5em" class="imageupload bntGuidance" src="../../Content/themes/TestMaker/v2/multiplechoice_images_guidance_unchecked.svg" title="">';
                            itemBody += iconString;
                          itemBody += '<div name="typeGuidance" style="display: none; border: 1px solid var(--grey7);padding: 5px;" idReponse="' + responseidentifier + '">' + htmlGuidance + '</div>';
                        }
                      ...
                      itemBody += '<p style="color:var(--red); display: inline-block;">' + 'Correct Answer: ' + that._getCorrectValueInlineChoice(question.CorrectResponse, itemBody) + '</p>';
                    }
                });
            } else {
                var responseIdentifier = $(questionItemBody).attr('responseidentifier');
                                            ...
                                            if (question.CorrectResponse[j].key == responseIdentifier) {
                                                ...
                                                for (var i = 0; i < correctResponses.length; i++) {
                                                    ...
                                                    if (correctResponse == selectedAnswer) {
                                                      ...
                                                      itemBody += '<p class="correct-answer-teacher-review" style="color:var(--red); display: inline-block;" idresponse="' + resId + '">' + 'Correct Answer: ' + correctResponseText + '</p>';
                                                        isCheck = true;
                                                    }
                                                }
                                                if (isCheck == false) {
          ...
          var isAutoOverridedClass = (overridden && isReviewed) ? 'is-auto-overrided' : '';
          itemBody = itemBody.replace('inlineChoiceFormat', "inlineChoiceFormat " + isAutoOverridedClass);
        ...
        _renderTextEntryResult: function (currentItemBody, self, question) {
            ...
            tree.find('textEntryInteraction').replaceWith(function () {
                                        ...
                                        if (objEntry.identifier === identifier) {
                                            for (var k = 0, lenArrMessage = objEntry.arrMessage.length; k < lenArrMessage; k++) {
                                                if (objEntry.arrMessage[k].typeMessage === 'rationale' || objEntry.arrMessage[k].typeMessage === 'guidance_rationale') {
                                                    iconString = '<img name="textEntryGuidance" responseidentifier="' + identifier + '" alt="Guidance" style="margin-left: 5px; width: 1.5em; height: 1.5em" class="imageupload bntGuidance" src="../../Content/themes/TestMaker/v2/multiplechoice_images_guidance_unchecked.svg" title="">';
                                                }
                                            }
                                            break;
                                        }
                ...
                var reviewableIcon = '';
                var isAutoOverridedClass = (overridden && isReviewed) ? ' is-auto-overrided' : '';
                newTextEntry.addClass('fill-in-blank' + isAutoOverridedClass);
