(function ($) {
    $.widget('jquery.QuestionRender', {
        ...
        ScrollToFirstManuallyGradedItem: function (question) {
            ...
            if (question.QTIItemSchemaID() == 21) {
                ...
                if ((viewModel.ManuallyGradedOnly() || (!viewModel.ManuallyGradedOnly() && !viewModel.OverrideAutoGraded())) && $manuallyGraded.length) {
                  ...
                  var questionNotReviewed = allSubQuestion.filter(function (index, element) {
                    return !$(element).is('.is-manually-graded, .is-auto-overrided, .sbSelector') 
                      || element.outerHTML.includes('red-border') 
                      || ($(element).is('.sbSelector') && !$(element).parent().prev().is('.is-auto-overrided'));
                    });

                    $firstManuallyGraded = (questionNotReviewed.length ? questionNotReviewed : allSubQuestion).first();
                }
                ...
                if (self.IsScrollToFirstManuallyGradedItem()) {
                    if (offsetFirstManually - offsetParent > 0) {
                        SCROLL_HEIGHT = offsetFirstManually + $assignmentQuestion.scrollTop() - offsetParent;
                    }
