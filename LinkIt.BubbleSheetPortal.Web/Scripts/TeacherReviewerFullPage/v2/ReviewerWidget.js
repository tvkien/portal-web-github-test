  ...
  $.widget('jquery.ReviewerWidget', {
    ...
    ShowReferenceContent: function (content, answerImage, refObjectID, refObjectData) {
      ...
      if ($passage.text().trim() === '' && !$passage.find('img').length) {
        ...
        passageHtml = '<div id="divPassageDetail">' + passageContent + '</div>';
        ...
        Reviewer.popupAlertMessage(passageHtml, 'ui-popup-fullpage', 700, 500, false);
      }
      var $referencePassage = $(document).find('#divPassageDetail');
      if(Reviewer.IsNullOrEmpty(refObjectID) && !Reviewer.IsNullOrEmpty(refObjectData)){
        // is passage import from Qti3p
        $referencePassage.addClass('is-qti3p-passage')
      }
      ...
      $.ajax({
        ...
        if (attachment.FileContent && attachment.FileContent.length) {
          if (canPlayback) {
            ...
            contentHtml += '<p style="font-weight: 700;margin-bottom: 4px;">' + attachment.FileName + '</p>';
            contentHtml += '</div>';
        ...
        success: function (response, status, xhr) {
          ...
          if (!options.WidgetUtil.IsNullOrEmpty(updatedAnswer)) {
            ...
            if (options.WidgetUtil.IsNullOrEmpty(self.AnswerSubID())) {
              ...
              var isManuallyGradedOnlyMode = (self.ManuallyGradedOnly() || (!self.ManuallyGradedOnly() && !self.OverrideAutoGraded()));
              var notYetReviewedAnswerSub = ko.utils.arrayFirst(
                updatedAnswer.TestOnlineSessionAnswerSubs(),
                function (answerSub) {
                  return !answerSub.IsReviewed() || (!isManuallyGradedOnlyMode && !answerSub.Overridden());
                }
              );
          ...
          var checkNoAutoQuestionNotOverried = true;
          if (!viewModel.ManuallyGradedOnly()) {
            var remainQuestion = ko.utils.arrayFirst(self.Questions(), function (question) {
              var isAutoGradedNotOverried = !question.Reviewable() && !viewModel.ManuallyGradedOnly() && !question.Answer().Overridden() && viewModel.OverrideAutoGraded();
              return question.VisibleQuestion() && isAutoGradedNotOverried;
            });
            checkNoAutoQuestionNotOverried = Reviewer.IsNullOrEmpty(remainQuestion);
            if (self.GradingType() == 'item' && viewModel.OverrideAutoGraded()) {
              checkNoAutoQuestionNotOverried = false;
            }
          }

          if (
            self.SubmitTestButtonCss() &&
            checkNoAutoQuestionNotOverried &&
    ...
    SelectNextApplicableStudent: function (self, isPendingReview, answerSubID) {
      ...
      if (self.SelectedStudentFilter() == self.AllStudentFilter().FilterValue() ||
        ...
        if (self.PendingReviewStudentFilter().FilterCount() > 0) {
          var pendingStudentIDs = '';
          if (isPendingReview && self.GradingType() == 'item' && !self.ManuallyGradedOnly() && self.OverrideAutoGraded()) {
            pendingStudentIDs = self.GetVisibleStudents().map(function (student) {
          ...
          $.post(options.GetNextApplicableStudentURL, {
            qtiTestClassAssignmentId: self.QTITestClassAssignmentID(),
            virtualQuestionId: self.SelectedQuestion().VirtualQuestionID(),
            IsManuallyGradedOnly: self.ManuallyGradedOnly(),
            ...
            if (pendingStudentIDs) {
              selectedStudent.CanBulkGrading(self.Questions().every(e => e.Answer().Overridden()));
      ...
      if (self.IsComplete() || selectedStudent.CanBulkGrading()) {
        ...
        var lastQuestion = [];
        var listCurrentQuestion = (self.ManuallyGradedOnly() || (!self.ManuallyGradedOnly() && !self.OverrideAutoGraded()
        )) ? self.GetReviewableQuestions() : self.Questions();
        if (listCurrentQuestion.length > 0) {
          lastQuestion = listCurrentQuestion.reduce(function (prev, current) {
            return prev.QuestionOrder() > current.QuestionOrder() ? prev : current;
          });
        }
        if (!selectedQuestion.Reviewable() && viewModel.ManuallyGradedOnly()) {
          nextQuestion = ko.utils.arrayFirst(self.Questions(), function (question) {
        ...
        if (selectedQuestion.QuestionOrder() != lastQuestion.QuestionOrder()) {
          ...
          if (selectedStudent.StudentOrder() == lastStudent.StudentOrder()) {
            nextQuestion = ko.utils.arrayFirst(viewModel.ManuallyGradedOnly() ? self.GetReviewableQuestions() : self.Questions(), function (question) {
          ...
          if (selectedStudent.StudentOrder() == lastStudent.StudentOrder()) {
            nextQuestion = (viewModel.ManuallyGradedOnly() ? self.GetReviewableQuestions() : self.Questions())[0];
            nextStudent = self.GetReadySubmitAndCompleteStudents()[0];
          }
        }
    ...
    SelectNextApplicableQuestion: function (self, isPendingReview, answerSubID) {
      ...
      if (self.SelectedStudentFilter() == self.AllStudentFilter().FilterValue() ||
        ...
        var nextQuestion = ko.utils.arrayFirst(self.Questions(), function (question) {
          var isAutoGradedNotOverried = !question.Reviewable() && !viewModel.ManuallyGradedOnly() && !question.Answer().Overridden() && viewModel.OverrideAutoGraded();
          var isManuallyGradedNotReviewed = question.Reviewable() && !question.IsReviewed();
          return question.VisibleQuestion() && (isManuallyGradedNotReviewed || isAutoGradedNotOverried);
        });
      ...
      if (self.IsComplete() || selectedStudent.CanBulkGrading()) {
        ...
        var nextQ = ko.utils.arrayFirst(self.Questions(), function (question) {
          var isAutoGradedNotOverried = !question.Reviewable() && !viewModel.ManuallyGradedOnly() && viewModel.OverrideAutoGraded();
          return question.VisibleQuestion() && question.QuestionOrder() > selectedQuestion.QuestionOrder() && (question.Reviewable() || isAutoGradedNotOverried);
        });
        if (nextQ == null) {
          ...
          if (nextS) {
            nextQ = ko.utils.arrayFirst(self.Questions(), function (question) {
              var isAutoGradedNotOverried = !question.Reviewable() && !viewModel.ManuallyGradedOnly() && viewModel.OverrideAutoGraded();
              return question.VisibleQuestion() && (question.Reviewable() || isAutoGradedNotOverried);
            });
          }
        }
    ...
    SelectNextApplicableSubPart: function (self, subID) {
      ...
      var $allSubQuestion = $assignmentQuestion.find('.box-answer, .is-manually-graded, .sbSelector, .fill-in-blank');
      var $questionList;

      if ((viewModel.ManuallyGradedOnly() || (!viewModel.ManuallyGradedOnly() && !viewModel.OverrideAutoGraded())) && $manuallyGraded.length) {
        $questionList = $manuallyGraded;
      } else {
        $questionList = $allSubQuestion;
      }

      for (var i = 0; i < $questionList.length; i++) {
        if ($questionList[i].outerHTML.indexOf(subID) > 0) {
          nextSubIndex = i + 1;
          break;
        }
      }

      if (nextSubIndex <= $questionList.length - 1)
        $firstManuallyGraded = $questionList[nextSubIndex];
      else
        return false;
      ...
      if (answerSubelement.hasClass('sbSelector')) {
        answerSubelement.next('.sbOptions').hide();
        answerSubelement.prev('.sbToggle').removeClass('sbToggleOpen');
      }
      ...
      if (offsetFirstManually - offsetParent > 0) {
        $assignmentQuestion.animate({
          scrollTop: offsetFirstManually + $assignmentQuestion.scrollTop() - offsetParent
        }, 500);
      }
    ...
    CanGrading: function (qtiOnlineTestSessionID, mode, selectedQuestion, answerID, pointsEarned,
      ...
      if (!Reviewer.IsNullOrEmpty(selectedQuestion) && (selectedQuestion.IsInformationalOnly() || selectedQuestion.IsAllOrNothingGrading()) ) {
        result = false;
      }
      return result;
    },
    CanEnableGradingShotcut: function (qtiOnlineTestSessionID, mode, selectedQuestion, answerID, pointsEarned,
      ...
      var result = Reviewer.IsNullOrEmpty(qtiOnlineTestSessionID) || (qtiOnlineTestSessionID > 0 && answerID > 0 && pointsEarned >= 0);
    ...
    PrintTestOfStudentSuccess: function (obj, callback) {
      ...
      dialogHtml += '<div style="font-weight: 700">' + obj.message;
      dialogHtml += '</div></div>';
      dialogHtml += '<div class="popup-fullpage-controls">';
      dialogHtml += '<button id="btn-ok-' + now + '" class="u-w-60" style="height: 45px;">OK</button>';
      dialogHtml += '</div>';
      dialogHtml += '</div>';
    ...
    ConfirmBulkSubmitTest: function (self, msg) {
      ...
      contentHtml += '<div class="popup-fullpage">';
      contentHtml += '<div class="popup-fullpage-content">';
      contentHtml += '<p class="u-text-left" style="font-weight: 700">' + msg + '</p>';
      contentHtml += '</div>';
      contentHtml += '<div class="popup-fullpage-controls">';
      contentHtml += '<button id="btn-bulk-cancel-' + now + '"class="btn-bulk" style="background: none;" >Cancel</button>';
