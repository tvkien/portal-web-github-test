...
function Question(data) {
  ...
  self.IsAllOrNothingGrading = ko.computed(function () {
    var rawXmlContent = self.XmlContent();
    var result = false;

    if (rawXmlContent) {
      var $tree = $(rawXmlContent);
      var mode = $tree.find('multiPartGradingSetting').attr('mode');

      if (mode === 'all-or-nothing-grading') {
        result = true;
      }
    }
    return result;
...
function TeacherReviewerViewModel(options) {
  ...
  self.GradingType.subscribe(function (newGradingType) {
    ...
    if (self.IsFullScreen()) {
      ...
      for (var i = 0; i < elListQuestion.length; i++) {
        elListQuestion[i].style.width = 'calc(100% - 70px)';
        elListQuestion[i].style.height = '100%';
      }
    }
  });
  ...
  self.ShowHightLight = function (highlightQuestion, question) {
    ...
    tree.find('.mainBody .questionNumber').remove();
  ...
  self.SelectedStudentID.subscribe(function (newStudentID) {
    if (Reviewer.IsNullOrEmpty(self.SelectedStudentID()) || self.SelectedStudentID() === -1) {
      self.ReviewerWidget.ReviewerWidget('ResetReviewer', self);
      !self.isFirstOpenPrintTestOfStudentPopup() && self.setDefaultSelectAllStudentOption();
      return;
    }

    $("#studentCurrent").prop("disabled", false);
    $("#studentCurrent").next().removeClass("text-muted");
  ...
  self.CheckFeedbackDirty = function () {
    ...
    if (self.OldFeedbackQuestion().trim() !== self.FeedbackQuestion().trim() || self.HasChangedTeacherAttachment()) {
      ...
      msg += msgDirty;
      Reviewer.popupAlertMessage(msg, 'ui-popup-fullpage ui-popup-fullpage-nostudent', 500);
      return false;
    }
  ...
  self.isFirstOpenPrintTestOfStudentPopup = ko.observable(true);
  ...
  self.OpenPrintTestOfStudentPopup = function () {
    ...
    if (self.isFirstOpenPrintTestOfStudentPopup()) {
      self.isFirstOpenPrintTestOfStudentPopup(false);

      if (Reviewer.IsNullOrEmpty(self.SelectedStudentID()) || self.SelectedStudentID() === -1) {
        self.setDefaultSelectAllStudentOption();
      }
    }
    self.IsOpenPrintTestOfStudentPopup(true);
  };

  self.setDefaultSelectAllStudentOption = function () {
    $("#studentCurrent").prop("disabled", true);
    $("#studentCurrent").next().addClass("text-muted");
    $("#studentCurrent").prop("checked", false);
    $("#studentAll").prop("checked", true).trigger("change");
  };
  ...
  self.IsDisabledGradingShortcutsGradedOnly = ko.computed(function () {
    return self.SelectedStudentID() === -1;
  });
  ...
  self.ShortcutReset = function () {
    ...
    self.ShortcutGradeBy(self.SelectedStudentID() === -1 ? 'all' : null);
    $("#portal-v2-containter .popup-fullpage-grading-shortcuts input").removeClass("input-checked-v2")
  }
  ...
  self.ToggleFullScreen = function () {
    ...
    if (elBody.classList.contains('teacherReviewerFullscreen')) {
      ...
      elDescQuestion.style.width = '100%';
    } else {
...
function getFullNameOnly(text) {
  var regex = /\((.*)\)/;
  var fullName = regex.exec(text)[1];
