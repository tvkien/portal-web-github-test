var AttachmentSettingComponent = Vue.extend({
  template: `
  <div v-if="allowShowAttachmentSetting" class="attachment-setting-wrapper">
   <p class="text-bold text-navy mb-2 me-3 text-nowrap">Attachment Options</p>
   <div class="row g-3 pe-4 row-checkbox-group">
      <div class="col-12 mb-2">
         <div class="center-checkbox">
            <input id="attachmentSetting-allowStudentAttachment" type="checkbox" v-model="attachmentSetting.allowStudentAttachment"/>
            <label for="attachmentSetting-allowStudentAttachment"> Allow Student Attachment</label>
            <span class="icon icon-info with-tip align-left d-inline-block" data-html="true" title="{{warningFileSize}}">
            </span>
         </div>
      </div>
      <div v-if="showRequireAttachment" class="col-12 mb-2">
        <div class="center-checkbox"><input id="attachmentSetting-requireAttachment" type="checkbox" v-model="attachmentSetting.requireAttachment" /><label for="attachmentSetting-requireAttachment"> Require Attachment</label></div>
      </div>
   </div>
   <div class="row g-4 row-select-group">
      <!--begin allow student attachment and Recording Options-->
      <div class="col-6" v-if="enableAttachmentDetailConfiguration">
          <label>Recording Options: </label>
          <select id="recording-options-select" multiple="multiple" class="full-width">
          </select>
      </div>
      ...
      <div class="col-6" v-if="enableAttachmentDetailConfiguration">
          ...
          <select id="attachment-types-select" multiple="multiple" class="full-width">
          </select>
        </div>
  ...
  methods: {
    ...
    loadAssessmentArtifactFileTypeGroups: function() {
      this.assessmentArtifactFileTypeGroups = this.assessmentArtifactConfiguration.assessmentArtifactFileTypeGroups.map(function(f) {
        return { displayName: f.displayName, name: f.name.toLowerCase(), supportFileType: f.supportFileType, maxFileSizeInBytes: f.maxFileSizeInBytes }
    ...
    handleFormatWarningFileSize(arrFileSizeGroup) {
      ...
      if (arrFileSizeGroup != null && arrFileSizeGroup.length > 0) {
        ...
        arrFileSizeGroup.map(function (m) { return html += ('<span style="font-size: 0.875rem; font-weight: 400; font-style: normal; text-align: left">' + m.displayName + ' - ' + self.onBytesToSize(m.maxFileSizeInBytes) + ' file size limit.' + '&nbsp</span></br>'); })
        html += '</span>';
      }
      return html;
    },
