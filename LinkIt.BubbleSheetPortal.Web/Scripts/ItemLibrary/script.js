...
$(document).ready(function () {
  ...
  function getAjaxSource1(isLoad = false) {
    ...
    if (isLoad) {
      return getAjaxLoadItem3rdPartyFromItemLibraryByFilter();
    }
  }
  ...
  function inItDataNWEATable() {
    var options1 = {
      ...
      aoColumns: [
        { sType: 'integer', sName: 'QTI3pItemID', bSearchable: false, bSortable: false, sWidth: "40px", sClass: 'col-action' },
        { sType: 'integer', sName: 'QTI3pItemID', bSearchable: false, bSortable: false, sWidth: "40px", sClass: 'col-action' },
        { sType: 'string', sName: 'Title', bSearchable: false, bSortable: false, sWidth: "120px" },
        { sType: 'string', sName: 'Description', bSearchable: false, bSortable: false, sWidth: "150px"},
        { sType: 'string', sName: 'BankName', bSearchable: false, bSortable: false, sWidth: "70px" },
        { sType: 'string', sName: 'GroupName', bSearchable: false, bSortable: false, sWidth: "70px" },
        { sType: 'string', sName: 'Standard', bSearchable: false, bSortable: false, sWidth: "100px" },
        { sType: 'string', sName: 'DistrictTag', bSearchable: false, bSortable: false, sWidth: "50px" },
      ...
      fnRowCallback: function (nRow, aData) {
        $('td:eq(0)', nRow).html(setCheckBoxItemNew3rd(aData[0]));
        $('td:eq(1)', nRow).html(setActionIcon3rd(aData[0]));
        if (aData[11] == 'True' || aData[11] == 'true') {
          var title = '';
          aData[8] = correctInlineChoice(aData[8]);
          $(aData[8]).find('.itemBody, itemBody, itembody').each(function () {
            var itemBody = $(this);
            itemBody.find("videolinkit").replaceWith(function () {
              return $('');
            });
            if ($(aData[8]).find("responsedeclaration").attr("partialgrading") == "1") {
              itemBody.find("sourcetext").each(function () {
                if ($(this).attr("pointvalue") > 0) {
                  $(this).addClass("marker-correct");
                }
              ...
              $(aData[8]).find("correctResponse").each(function () {
                var id = $(this).attr("identifier");
                itemBody.find("sourcetext[identifier=\"" + id + "\"]").addClass("marker-correct");
              });
            }
          ...
          $('td:eq(8)', nRow).html(divTitle);
        }
        else {
          $('td:eq(8)', nRow).html(parseXmlContent1(aData[8]));
        }
        $('td:eq(8)', nRow).attr("onclick", 'showEditQti3pItemPopup(' + aData[0] + ',1)');
        $('td:eq(8)', nRow).addClass('with-tip');
        $('td:eq(8)', nRow).bind({
          mouseenter: function () {
            displayItemTooltip($(this), aData[9], aData[10]);
          },
          mouseleave: function () {
        ...
        $('td:eq(4)', nRow).css("background-color", "var(--disabledColorBg)");
        $('td:eq(5)', nRow).css("background-color", "var(--disabledColorBg)");
        $('td:eq(7)', nRow).css("background-color", "var(--disabledColorBg)");
      },
      ...
      fnDrawCallback: function () {
        ...
        if ($('#chkAllItem') && $('#chkAllItem').length > 0) {
          $('#chkAllItem').removeAttr('checked');
          $('#chkAllItem').removeClass('input-checked-v2');
        }
        if ($('#showPassagesForFoundItem') && $('#showPassagesForFoundItem').length > 0) {
      ...
      fnInitComplete: function () {
        tranformSearchInputDataTable('dataTable1_filter')
        $('#dataTable1_wrapper .data-table-action').prepend(`
             <img 
        ...
        if (divOnTop) {
          var filteredLabel = $('#filteredLabel3rd');
  ...
  $('input[name="ItemLibrarySelection"]').click(function () {
    ...
    $('#showPassagesForFoundItem').prop('disabled', true);
    $('#btnAddPassage').prop('disabled', true);
    $('#btnAssignTag').prop('disabled', true);
    $('#btnAssignStandard').prop('disabled', true);
    if ($(this).attr('id') == 'radioNWEAItemLibrary') {
      $('.linkit-tab').hide();
      $('#btnAddPassage').hide();
      $('#btnAssignTag').hide();
      $('#btnAssignStandard').hide();
      isNWEAItemTab = true;
      ...
      firstLoadListItemsFromLibrary = false;
      // Reset paging per page
      var oTable = $('#dataTable1').dataTable();
      oTable.fnSettings()._iDisplayLength = 100;
      oTable.fnDraw();
      $('[name=dataTable1_length]').val(100);
    }
    else {
      $('.linkit-tab').show();
      $('#btnAddPassage').show();
      $('#btnAssignTag').show();
      $('#btnAssignStandard').show();
      if (!firstLoad) {
      ...
      oTable.fnSettings()._iDisplayLength = 100;
      oTable.fnDraw();
      $('[name=dataTable2_length]').val(100);
    }
...
$(function () {
  ...
  $("#setFilter").click(function () {
    ...
    var settings = $("#dataTable2").dataTable().fnSettings()._iDisplayLength;
    var dropdown = $('[name=dataTable2_length]').val();
    if (settings != dropdown)
      $('[name=dataTable2_length]').val(settings);
