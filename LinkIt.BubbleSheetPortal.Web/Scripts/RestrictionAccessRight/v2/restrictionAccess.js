...
var URL_STATE = url + '/PopulateStateDistrict/GetStates',
  ...
  URL_GET_RESTRICTION_BY_RESTRICTEDOBJECT = url + '/RestrictionAccessRights/GetRestrictionByRestrictedObject';
...
function loadEditAccessRights(options) {
  ...
  template = template.replace('{{DisplayTitle}}', options.DisplayName + ' Restrictions')
    ...
    .replace('{{TeacherAccessLabelContent}}', 'Allow Teacher access to ' + displayType + ' content')
    .replace('{{SchoolAdminAccessLabelContent}}', 'Allow School Admin access to ' + displayType + ' content')
  ...
  var checkedSchoolContent = options.SchoolAdminRestrictionContent == RESTRICTION_TYPE.FULL_ACCESS;
  var checkedTeacherContent = options.TeacherRestrictionContent == RESTRICTION_TYPE.FULL_ACCESS;

  if (!options.CategoryTestId) {
    var allTeachers = [];
    var allSchools = [];
    var allTeacherContents = [];
    var allSchoolContents = [];

    $listItems.each(function () {
      allTeachers.push($(this).attr('teacherRestrictAccess'));
      allSchools.push($(this).attr('schoolAdminRestrictAccess'));
      allTeacherContents.push($(this).attr('teacherRestrictAccessContent'));
      allSchoolContents.push($(this).attr('schoolAdminRestrictAccessContent'));
    });
    ...
    checkedSchoolContent = allSchoolContents.indexOf(RESTRICTION_TYPE.NO_ACCESS) == -1;
    checkedTeacherContent = allTeacherContents.indexOf(RESTRICTION_TYPE.NO_ACCESS) == -1;
  }
  ...
  /*** Check condition enable or disable checkbox content */
  $container.find("#chkTeacherAccessContent").attr("disabled", !checkedTeacher);
  $container.find("#chkSchoolAdminAccessContent").attr("disabled", !checkedSchool);
  $container.find('#chkTeacherAccessContent').attr('checked', checkedTeacherContent);
  $container.find('#chkSchoolAdminAccessContent').attr('checked', checkedSchoolContent);
  ...
  if (options.RestrictionTypeName == 'test') {
    ...
    if (category && category.SchoolAdminRestrictionContent == RESTRICTION_TYPE.NO_ACCESS) {
      $container.find("#chkSchoolAdminAccessContent").attr('disabled', true);
      $container.find('#chkSchoolAdminAccessContent').attr('checked', false);
    }
    if (category && category.TeacherRestrictionContent == RESTRICTION_TYPE.NO_ACCESS) {
      $container.find("#chkTeacherAccessContent").attr('disabled', true);
      $container.find('#chkTeacherAccessContent').attr('checked', false);
    }
  }

  /*** Add event checked teacher access or school admin access */
  $container.find("#chkTeacherAccess").change(function (event) {
    if (options.RestrictionTypeName == 'test') {
      if (category && category.TeacherRestrictionContent == RESTRICTION_TYPE.NO_ACCESS) {
        $container.find("#chkTeacherAccessContent").attr('disabled', true);
      }
      else {
        $container.find("#chkTeacherAccessContent").attr("disabled", !event.target.checked);
  ...
  $container.find("#chkSchoolAdminAccess").change(function (event) {
    if (options.RestrictionTypeName == 'test') {
      if (category && category.SchoolAdminRestrictionContent == RESTRICTION_TYPE.NO_ACCESS) {
        $container.find("#chkSchoolAdminAccessContent").attr('disabled', true);
      }
      else {
        $container.find("#chkSchoolAdminAccessContent").attr("disabled", !event.target.checked);
        if (!event.target.checked) {
          $container.find("#chkSchoolAdminAccessContent").attr('checked', false);
        }
      }
