...
var app = new Vue({
  ...
  data: {
    ...
    schoolId: !sessionStorage.getItem('schoolId') ? 0 : JSON.parse(sessionStorage.getItem('schoolId')),
    disabled: true,
    disabledFilter: true,
    roles: _roles,
    userRole: RoleID,
  ...
  created: function () {
    ...
    if (!isSearchBefore) {
      ...
      sessionStorage.setItem('schoolId', 0);
    ...
    that.schoolId = sessionStorage.getItem('schoolId') ? JSON.parse(sessionStorage.getItem('schoolId')) : 0;
  ...
  methods: {
      ...
      if (this.selectedDistrictId !== 0 || _selectedDistrictId) {
        ...
        GetSchoolByDistrictId(URL_SCHOOL + (_selectedDistrictId ? _selectedDistrictId : this.selectedDistrictId), function (school) {
          if (school.length === 1) {
            ...
            that.schoolId = that.school[0].id;
            that.selectSchool();
          } else {
            if (school.length === 0) {
              that.school = [];
    ...
    selectSchool: function (e) {
      if (this.selectedSchool !== 0) {
        ...
        this.schoolId = parseInt(selected.value);
      } else {
        this.schoolId = 0;
      }
    },
    onFilterSchool: function () {
      ...
      if (isApplyFilter) {
        ...
        sessionStorage.setItem('schoolId', JSON.stringify(this.schoolId));
      ...
      loadDistrictDetailData(this.selectedDistrictId, this.schoolId);
    },
