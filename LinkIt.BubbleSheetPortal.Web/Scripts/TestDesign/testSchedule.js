...
function testSchedule(systemDateFormat) {
    ...
    this.validateTestScheduleForm = function () {
        ...
        if ($('.test-schedule-action[value="1"]:checked').length > 0) {
    ...
    function initViewDefaultData() {
        var fromDate = new Date();
        $("#testSchedule0").attr("checked", "checked");
        $("input[name='testScheduleFromDateView']").datepicker("setDate", fromDate);
        $("input[name='testScheduleToDateView']").datepicker("setDate",new Date(fromDate.setMonth(fromDate.getMonth() + 1)));
        $("input[name='testScheduleStartTimeView']").val("00:00");
        $("input[name='testScheduleEndTimeView']").val("23:59");
        $("input[name='testScheduleDayOfWeekView']").attr("checked", "checked");
    }
            ...
            switch (tag.Key) {
                case 'testSchedule':
                    if (tag.Tooltips && tag.Tooltips !== '') {
                      $('input[name="testSchedule"]').parent().siblings(":first").append("<a href='javascript:void(0)' title='" + tag.Tooltips + "' class='with-tip'><img src = '/Content/themes/Constellation/images/icons/fugue/information-blue.png' width = '16' height = '16' ></a >");
                    } 
                    if (tag.Value == 1) {
                        $("#testSchedule1").click();
                    ...
                    if (tag.Attributes && tag.Attributes.length > 0) {
                        $.each(tag.Attributes, function (attrIndex, attribute) {
                            if (attribute.Key == 'lock' && attribute.Value == 'true') {
                                $('#chklockTestSchedule').attr('checked', "checked");
    ...
    function bindingDataView(data) {       
        ...
        $.each(data, function (index, tag) {
            switch (tag.Key) {
                ...
                case 'testScheduleDayOfWeek':
                    var days = tag.Value.split("|");
                    var allDays = $("input[name='testScheduleDayOfWeekView']");
    ...
    function defineButtonFunctions() {
      ...
      $('.test-schedule-action').on("click", function () {
        ...
        $("#btnSaveSchedule").on("click", function () {
            ...
            var days = $("input[name='testScheduleDayOfWeekView']:checked");
            var testScheduleDayOfWeekValue = [];
    ...
    function initControlAndEventForView() {
        $("#testScheduleFromDateView").datepicker();
        ...
        $("input[name='testScheduleStartTimeView']").change(function () {
            if($(this).val() == "00:00" && $("input[name='testScheduleEndTimeView']").val() == "23:59"){
                $("#alway-open").prop("checked", true);
            } else{
                $("#alway-open").prop("checked", false);
            }
        ...
        $("input[name='testScheduleEndTimeView']").change(function () {
            if($("input[name='testScheduleStartTimeView']").val() == "00:00" && $(this).val() == "23:59"){
                $("#alway-open").prop("checked", true);
            } else{
                $("#alway-open").prop("checked", false);
            }
