...
var app = new Vue({
  ...
  computed: {
    ...
    showDistrictAdminRole: function () {
      switch (this.selectedLevel) {
        case 'school':
  ...
  methods: {
    ...
    getPositionTagIt: function () {
      var selectWidth = $('#selectClasses').outerWidth() || 247;
      var selectHeight = $('#selectClasses').outerHeight() || 41;
      var selectPosition = $('#selectClasses').offset();
      var selectPosition = $('#selectClasses').offset();
      var elSelectField = $('ul.ui-autocomplete.tagit-autocomplete').get(0).style;
      elSelectField.setProperty('max-width', selectWidth + 'px');
      elSelectField.setProperty('--positionLeft', selectPosition.left + 'px');
      elSelectField.setProperty('--positionTop', (selectPosition.top + selectHeight) + 'px');
    },
    populateDataToTagIt: function (availableTagNames, tagitId, hint) {
      ...
      $('#selectClasses .tagit-dropdown').on('click', function () {
        self.getPositionTagIt();
      })

      $('#selectClasses input[type="text"]').focus(function () {
        self.getPositionTagIt();
      })

      $('ul.tagit input[type="text"]').css("min-width", "10px");
    },
    removedInputCheckV2: function (filterName) {
      var elCheckAll = $('input' + filterName);
      if (elCheckAll.hasClass('input-checked-v2')) {
        elCheckAll.removeClass('input-checked-v2');
      }
    },
    ...
    selectCheckBoxTestTypes: function () {
      this.checkTestTypes = false;
      this.removedInputCheckV2('#test-types');
    ...
    selectCheckBoxGrades: function () {
      this.checkGrades = false;
      this.removedInputCheckV2('#grades');
    ...
    selectCheckBoxSubjects: function () {
      this.removedInputCheckV2('#subjects');
