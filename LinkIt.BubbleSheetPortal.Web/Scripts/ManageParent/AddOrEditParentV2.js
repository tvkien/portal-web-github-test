  ...
  methods: {
    ...
    showAssignStudentPopup: function () {
      if ((this.parentData.districtId || 0) < 1) {
        CustomAlert('Please select a District first');
        return;
      }
      var self = this;
      ...
      setTimeout(function () {
        ...
        $.get(self.getAvailableProgramAndGradeurl, { parentUserId: self.parentData.userId, districtId: self.parentData.userId })
          .done(function (response) {
            ...
            var isCheckedSelectedOnly = $("#cbShowSelectedStudent").is(':checked');
            if (!isCheckedSelectedOnly) {
              $("#cbShowSelectedStudent").removeClass("input-checked-v2")
            };
            $("#cbShowSelectedStudent").prop('disabled', false)
          });
      }, 200)
    },
