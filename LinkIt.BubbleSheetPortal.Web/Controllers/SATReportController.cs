...
{
    public class SATReportController : BaseController
    {
        ...
        public SATReportController(ACTReportService actReportService, UserService userService,
        ...
        private void BuildSessionTagData(SATReportMasterViewModel masterModel, int studentID, int testId)
        {
            ...
            if (masterModel.DiagnosticHistoryViewModel.TestAndScoreViewModels.Any(en => en.IsSelected))
            {
                ...
                foreach (var subjectId in listSubject)
                {
                    ...
                    var tagCategoryData = lastestSubjectTag.Where(x => x.SubjectID == subjectId).OrderBy(m => m.ItemTagCategoryOrder).ToList();
                    ...
                    var answerData = answerSectionData.Where(x => x.SubjectID == subjectId)
                        .GroupBy(m => m.VirtualQuestionID)
                        .Select(x => new AnswerSectionViewModel
                        {
                            WasAnswered = x.First().WasAnswered,
                            PointsEarned = x.First().PointsEarned,
                            PointsPossible = x.First().PointsPossible,
                            CorrectAnswer = ParseCorrectAnswer(x.First().CorrectAnswer),
                            AnswerLetter = x.First().AnswerLetter,
                            AnswerID = x.First().AnswerID,
                            QuestionOrder = x.First().QuestionOrder,
                            QTISchemaID = x.First().QTISchemaID,
                            SectionID = x.First().SectionID,
                            SectionName = x.First().SectionName,
                            VirtualQuestionID = x.First().VirtualQuestionID,
                            PassageIds = x.Select(m => m.PassageID).ToList(),
                            PassageName = string.Join(", ", x.Where(m => m.PassageID.HasValue).OrderBy(m => m.PassageID).Select(m => m.PassageID).Distinct())
                        }).ToList();

                    sectionTagModel.AnswerSectionViewModels = answerData;

                    if (sectionTagModel.SectionName.ToLower().Contains(SATConstants.WritingText))
                    {
                        var listPassages = answerData.Where(m => !string.IsNullOrEmpty(m.PassageName)).Select(m => m.PassageName).Distinct();

                        var answerDataGroupByPassage = new List<AnswerSectionGroupByPassageViewModel>();

                        foreach (var item in listPassages)
                        {
                            answerDataGroupByPassage.Add(new AnswerSectionGroupByPassageViewModel
                            {
                                PassageName = item,
                                AnswerSectionViewModels = answerData.Where(m => m.PassageName == item)
                                                            .Select(x => new AnswerSectionViewModel
                                                            {
                        ...
                        answerDataGroupByPassage.Add(new AnswerSectionGroupByPassageViewModel
                        {
                            AnswerSectionViewModels = answerData.Where(m => string.IsNullOrEmpty(m.PassageName))
                                                            .Select(x => new AnswerSectionViewModel
                                                            {
                                                                WasAnswered = x.WasAnswered,
                                                                PointsEarned = x.PointsEarned,
                        ...
                        sectionTagModel.AnswerSectionGroupByPassageViewModels = answerDataGroupByPassage;
                        sectionTagModel.IsDisplayPassageNumber = answerData.Any(m => m.PassageIds.Where(x => x.HasValue && x.Value > 0).Count() > 0);
                    }
                                          ...
                                          List<ACTAnswerSectionData> answerSectionData)
        {
            ...
            {
                var tagCategory = tagCategoryData.FirstOrDefault(x => x.CategoryID == categoryID);

                if (tagCategory != null)
                {
                    var tagCategoryModel = new TagCategoryReportViewModel();
                    tagCategoryModel.TagCategoryName = tagCategory.CategoryName;
                    tagCategoryModel.TagCategoryDescription = tagCategory.CategoryDescription;
                    tagCategoryModel.SingleTagReportViewModels = new List<SingleTagReportViewModel>();
                    tagCategoryModel.Order = tagCategory.ItemTagCategoryOrder;

                    if (tagCategory.PresentationType.HasValue)
                    {
                        tagCategoryModel.PresentationType = (ACTPresentationType)tagCategory.PresentationType.Value;
                    }
                                        ...
                                        List<ACTAnswerSectionData> answerSectionData)
        {
            ...
            foreach (var actSectionTagData in tagData)
            {
                ...
                var tag = tagData.Where(x => x.TagID == actSectionTagData.TagID);
                var tagModel = new SingleTagReportViewModel
                {
                    IncorrectAnswer = tag.Sum(x => x.IncorrectAnswer),
                    BlankAnswer = tag.Sum(x => x.BlankAnswer),
                    CorrectAnswer = tag.Sum(x => x.CorrectAnswer),
                    HistoricalAverage = Convert.ToInt32(tag.Average(x => x.HistoricalAvg)),
                    TotalAnswer = tag.Sum(x => x.TotalAnswer),
                    TagName = actSectionTagData.TagName,
                    TagNameForOrder = actSectionTagData.TagNameForOrder,
                    Percent = Convert.ToInt32(tag.Average(x => x.Percentage)),
                    ListAnswerInTag = new List<ACTAnswerSectionData>(),
                    Order = tag.FirstOrDefault() != null ? tag.FirstOrDefault().ItemTagOrder : null
                };
            ...
            tagCategoryModel.SingleTagReportViewModels =
                tagCategoryModel.SingleTagReportViewModels.OrderBy(en => en.Order).ToList();
        }
        ...
        public SATReportMasterViewModel BuildReportPrinting(int studentID, int districtID, int testID, int teacherID, int generatedUserID, int classID, bool isLastTest, int reportContentOption, int? takenReportTeacherId, int? takenReportClassId)
        {
            _districtID = districtID;
            Util.LoadDateFormatToCookies(districtID, districtDecodeService);
        ...
        private void IncludeJSCss(SATReportMasterViewModel model)
        {
            var cssPaths = new List<string>
            {
                ...
                "Content/css/reports/sat-chart.css"
            };
        ...
        public ActionResult ReportPrintingForCoding()
        {
            ...
            var masterModel = BuildReportMasterModel(1972899, 2621, 117160, 428203, 587605, 3232274, false, 1, 428203, 3751849);
        ...
        private byte[] PrintOneFileV2(List<int> listStudentIDs, int districtId, int testID, int teacherID, int timezoneOffset, int classID, int reportContentOption, out string reportUrl)
        {
            ...
            for (int index = 0; index < listStudentIDs.Count; index++)
            {
                ...
                var url = Url.Action("ReportPrinting", "SATReport",
                    new
                    {
                        studentID,
                ...
                var model = BuildReportPrinting(studentID, districtId, testID, teacherID, CurrentUser.Id, classID, isLastTest, reportContentOption, null, null);
                    ...
                    foreach (var listClassId in listClassIds)
                    {
                        ...
                        foreach (var virtualTestId in listVirtualTestIds)
                        {
                            ...
                            for (int index = 0; index < listReportStudentIds.Count; index++)
                            {
                                ...
                                var model = BuildReportPrinting(studentId, districtId, virtualTestId, listTeacherId, CurrentUser.Id, listClassId, false, reportContentOption, null, null);
                                if (reportContentOption != 3 || model.BubbleSheetFileSubViewModels.Count > 0)
                                {
                                    var html = this.RenderRazorViewToString("ReportPrinting", model);
                                    var pdf = ExportToPDFByPrinceXML(html);
                ...
                foreach (var classData in sumClassTestStudents)
                {
                    ...
                    foreach (var virtualTestData in classData.Value)
                    {
                        ...
                        foreach (var studentId in virtualTestData.Value)
                        {
                            ...
                            var model = BuildReportPrinting(studentId, districtId, virtualTestData.Key, currentTeacher.Id, CurrentUser.Id, classData.Key, false, reportContentOption, null, null);
                            if (reportContentOption != 3 || model.BubbleSheetFileSubViewModels.Count > 0)
                            {
                                var html = this.RenderRazorViewToString("ReportPrinting", model);
                                var pdf = ExportToPDFByPrinceXML(html);
            ...
            else// Print and group report based on selected class
            {
                ...
                foreach (var virtualTestData in testStudents)
                {
                    ...
                    foreach (var studentData in virtualTestData.Value)
                    {
                        ...
                        var model = BuildReportPrinting(studentId, districtId, virtualTestData.Key, currentTeacher.Id, CurrentUser.Id, takenTestClassId, false, reportContentOption, teacherId.Value, classId.Value);
                        if (reportContentOption != 3 || model.BubbleSheetFileSubViewModels.Count > 0)
                        {
                            var html = this.RenderRazorViewToString("ReportPrinting", model);
                            var pdf = ExportToPDFByPrinceXML(html);
        ...
        private string CreateNewSATFileAndUploadToS3(NewACTReportData model,
            List<ReportFileResultModel> reportFileResultModels)
        {
                ...
                else
                {
                    ...
                    foreach (var reportFileResultModel in reportFileResultModels)
                    {
                        ...
                        for (int i = 0; i < numOfFile; i++)
                        {
                            ...
                            {
                                if (i * specializedReportTestResultPerFileConfiguration + j <
                                    reportFileResultModel.PdfFiles.Count)
                                {
                                    onePdfFiles.Add(
        ...
        public ActionResult GenerateFromAPI(GenerateReportViewModel model)
        {
            ...
            if (!string.IsNullOrEmpty(model.ReportType) && model.TestResultId.HasValue && model.APIAccountID.HasValue)
            {
                ...
                var newSatReportData = BuildModelForGenerateNewACTStudentReport(model, apiAccount);
                result = GenerateNewSATStudentReport(newSatReportData);
            }
            return Json(result, JsonRequestBehavior.AllowGet);
        }

        private GenerateReportResultViewModel GenerateNewSATStudentReport(NewACTReportData model)
        {
            if (!model.DistrictId.HasValue)
            {
            ...
            var reportFileResultModels = PrintOneFileNewSAT(model.DistrictId.Value, model.SchoolId, model.TeacherId, model.DistrictTermId,
                ...
                , model.ResultDateFrom, model.ResultDateTo
                , model.SpecializedReportJobId
                , model.TimezoneOffset, model.ReportContentOption, model.isGetAllClass,
                out reportUrl);
