...
namespace LinkIt.BubbleSheetPortal.Web.Controllers
{
    ...
    [AjaxAwareAuthorize]
    [VersionFilter]
    public class DataLockerEntryResultController : BaseController
    {
        ...
        public DataLockerEntryResultController(DataLockerControllerParameters parameters, UserBankService userBankService, IS3Service s3Service, IDocumentManagement documentManagementService)
        {
            this._parameters = parameters;
            this._userBankService = userBankService;
        ...
        [HttpPost]
        [AjaxOnly]
        public ActionResult EntryResultTempDatas(EntryResultModel model)
        {
            if (model.BankId.HasValue)
            {
                model.SubjectId = _userBankService.GetSubjectIdOfABankByBankId(model.BankId.Value);
            }
            Session["EntryResultModel"] = model;
        ...
        public ActionResult EntryResults(int? virtualTestId, int? classId)
        {
            ...
            if (!model.VirtualTestId.HasValue || !model.ClassId.HasValue ||
                string.IsNullOrEmpty(model.StudentsIdSelectedString))
                return RedirectToAction("EnterResults");
            ...
            if (!hasPermissionOnClass)
                return RedirectToAction("EnterResults");
            var hasPermissionOnStudents = _parameters.VulnerabilityService.CheckUserPermissionOnStudent(CurrentUser, model.StudentsIdSelectedString);
            if (!hasPermissionOnStudents)
                return RedirectToAction("EnterResults");
            var hasStudentNotInClass = _parameters.ClassStudentService.HasStudentNotInClass(model.ClassId.GetValueOrDefault(), model.StudentsIdSelectedString);
            if (!hasStudentNotInClass)
                return RedirectToAction("EnterResults");
            ...
            try
            {
                ...
                int schoolId = EntryResultSession != null && EntryResultSession.SchoolId.HasValue && EntryResultSession.SchoolId > 0
                                ? EntryResultSession.SchoolId.Value
                                : classObject.SchoolId ?? 0;
                schoolId = (model.IsOldUI.HasValue && model.IsOldUI.Value == true) ? 0 : schoolId;
                //get data preference
        ...
        [HttpPost]
        public ActionResult DeleteAutoSaveData(int? virtualTestId, int? classId)
        {
            try
            {
                _parameters.DataLockerService.DeleteAutoSaveData(virtualTestId.GetValueOrDefault(),
                    classId.GetValueOrDefault(), CurrentUser.Id);
                return Json(true, JsonRequestBehavior.AllowGet);
            }
            catch
            {
        ...
        public ActionResult GetEntryStudents(int virtualTestId, int classId, string studentsIdSelectedString, bool? isOldUI)
        {
            ...
            var autoSave = _parameters.DataLockerService.GetAutoSaveData(virtualTestId, classId, CurrentUser.Id);
            if (autoSave != null)
            {
            ...
            var classObject = _parameters.ClassService.GetClassById(classId);
            //Check show pbs
            var EntryResultSession = (EntryResultModel)Session["EntryResultModel"];
            int districtFilter = EntryResultSession != null && EntryResultSession.DistrictId.HasValue && EntryResultSession.DistrictId > 0 ? EntryResultSession.DistrictId.Value : CurrentUser.DistrictId.Value;
            int schoolId = classObject.SchoolId ?? 0;
            ...
            if (studentIdGetting != null && studentIdGetting.Any())
            {
                ...
                _parameters.DataLockerService.GetStudentAndTestResultScore(virtualTestId, classId, string.Join(",", studentIdGetting));
                ...
                var studentTestResultScores = studentTestResultScoreQueryable
                ...
                var studentTestResultSubScoresQueryable =
                _parameters.DataLockerService.GetStudentAndTestResultSubScore(virtualTestId, classId, string.Join(",", studentIdGetting));
                documentGuidIds = studentTestResultSubScoresQueryable.Where(w => w.Artifacts != null).SelectMany(s => s.Artifacts?.Where(p => p.DocumentGuid != null && p.DocumentGuid != Guid.Empty)?.Select(art => art?.DocumentGuid));
                documentInfos = new List<DocumentInforDto>();
                if (documentGuidIds != null && documentGuidIds.Count() > 0)
                {
                    documentInfos = _documentManagementService.GetDocumentInfoList(documentGuidIds);
                }
                var studentTestResultSubScores = studentTestResultSubScoresQueryable.Select(x => new DTLStudentAndTestResultSubScore()
                {
                    ClassID = x.ClassID,
                    Code = x.Code,
                    AltCode = x.AltCode,
            ...
            model.Preferences = _parameters.PreferencesService.GetPreferenceEntryResultForm(virtualTestId, CurrentUser.DistrictId ?? 0, CurrentUser.StateId.GetValueOrDefault(), schoolId);
            model.QTITestClassAssignmentData = _parameters.QTITestClassAssignmentService.CheckUnPubLishForm(virtualTestId, classId);

            return new ContentResult() { Content = serializer.Serialize(model), ContentType = "text/json" };

        }        

        public ActionResult GetInfoPreviewForm(int virtualTestId)
        {
            var model = new ResultEntryInputScoreModel();
        ...
        public ActionResult LoadPublishForm(int virtualTestId, int classId, bool? isOldUI)
        {
            var classObject = _parameters.ClassService.GetClassById(classId);
            var schoolId = (isOldUI.HasValue && isOldUI.Value == true) ? 0 : (classObject.SchoolId ?? 0);
            var preference = _parameters.PreferencesService.GetPreferenceEntryResultForm(virtualTestId, CurrentUser.DistrictId ?? 0, CurrentUser.StateId.GetValueOrDefault(), schoolId);
            var obj = JsonConvert.DeserializeObject<DataSettings>(preference.Value);
            ViewBag.DateFormat = _parameters.DistrictDecodeService.GetDateFormat(CurrentUser.DistrictId ?? 0).JQueryDateFormat;
            ViewBag.Tooltips = _parameters.PreferencesService.GetDataLockerPreferenceLocalize(CurrentUser.DistrictId.GetValueOrDefault());
            ViewBag.ClassId = classId;
            ViewBag.VirtualTestId = virtualTestId;
            ViewBag.LastUpdatedDate = preference.UpdatedDate.DisplayDateWithFormat(true);
            return PartialView("_PublishForm", obj);
        }

        [HttpPost]
        public ActionResult CheckAutoSaveData(int? virtualTestId, int? classId)
        {
            var autoSave = _parameters.DataLockerService.GetAutoSaveData(virtualTestId.GetValueOrDefault(), classId.GetValueOrDefault(), CurrentUser.Id);
            return Json(autoSave != null, JsonRequestBehavior.AllowGet);
        }
        [HttpPost]
        public ActionResult AutoSaveResult(int? virtualTestId, int? classId, string studentTestResultScores, string studentTestResultSubScores, string actualTestResultScoresJson, DateTime? resultDate)
        {
            ...
            try
            {
                _parameters.DataLockerService.AutoSaveResult(data);
            }
            catch
            {
        ...
        [HttpPost]
        public ActionResult SaveResults(ResultEntrySaveResultModel resultEntrySaveResultModel)
        {
            var scores = resultEntrySaveResultModel.Scores
                .Where(x => resultEntrySaveResultModel.IsSaveForPublish || x.TestResultScore.TestResultID.HasValue || IsValidData(x))
                .ToList();
            ...
            if (scores.Any())
            {
                try
                {
                    ...
                    foreach (var item in scores)
                    {
                        ...
                        {
                            testResultScoreNoteList.AddRange(BuildTestResultScoreNoteViewModel(item.TestResultScore, lineIndex));
                        }
                        ...
                        if (item.TestResultSubScores != null)
                        {
                            foreach (var subItem in item.TestResultSubScores)
                            {
                                if (subItem != null)
                                {
                                    ...
                                    if (subScoreMeta != null)
                                    {
                                        testresultSubScore.UseRaw = subScoreMeta.UseRaw;
                                        testresultSubScore.UsePercent = subScoreMeta.UsePercent;
                                        testresultSubScore.UsePercentage = subScoreMeta.UsePercentile;
                                        testresultSubScore.UseScaled = subScoreMeta?.UseScaled;
                                        testresultSubScore.AchievementLevel = subScoreMeta.AchievementLevelSettingId;
                                        testresultSubScore.PointsPossible = subScoreMeta.PointsPossible;
                                        testresultSubScore.UseArtifact = subScoreMeta.UseArtifact;
                                    }
                                    ...
                                    {
                                        testResultSubScoreNoteList.AddRange(BuildTestResultSubScoreNoteViewModel(item.TestResultScore, subItem, lineIndex));
                                    }
                    ...
                    _parameters.DataLockerService.SaveEntryResults(
                    ...
                    //delete all autosave
                    _parameters.DataLockerService.DeleteAllAutoSaveData(virtualtestId.GetValueOrDefault(), classId.GetValueOrDefault());
                    return Json(new { Success = true, JsonRequestBehavior.AllowGet });
                }
                catch (Exception ex)
                {
        ...
        private List<TestResultScoreNoteViewModel> BuildTestResultScoreNoteViewModel(DTLStudentAndTestResultScore testResultScore, int lineIndex)
        {
        ...
        private List<TestResultSubScoreNoteViewModel> BuildTestResultSubScoreNoteViewModel(DTLStudentAndTestResultScore testResultScore, DTLStudentAndTestResultSubScore subScore, int lineIndex)
        {
        ...
        private bool IsValidData(ResultEntrySaveScoreModel item)
        {
            ...
            if (trScore != null && (trScore.ScoreRaw.HasValue || trScore.ScoreScaled.HasValue || trScore.ScorePercent.HasValue ||
                trScore.ScorePercentage.HasValue || !string.IsNullOrEmpty(trScore.ScoreCustomA_1) || !string.IsNullOrEmpty(trScore.ScoreCustomA_2) ||
                !string.IsNullOrEmpty(trScore.ScoreCustomA_3) || !string.IsNullOrEmpty(trScore.ScoreCustomA_4) || trScore.ScoreCustomN_1.HasValue || trScore.ScoreCustomN_2.HasValue
            ...
            if (item.TestResultSubScores != null)
            {
                ...
                foreach (var subItem in item.TestResultSubScores)
                {
                    ...
                    if (subItem != null && (subItem.ScoreRaw.HasValue || subItem.ScoreScaled.HasValue || subItem.ScorePercent.HasValue ||
                        subItem.ScorePercentage.HasValue || !string.IsNullOrEmpty(subItem.ScoreCustomA_1) ||
                        !string.IsNullOrEmpty(subItem.ScoreCustomA_2) ||
                        !string.IsNullOrEmpty(subItem.ScoreCustomA_3) || !string.IsNullOrEmpty(subItem.ScoreCustomA_4) ||
        ...
        public ActionResult LoadBankBySubjectId(FormBankCriteria criteria)
        {
            ...
            criteria.UsingMultiDate = _parameters.DistrictDecodeService.GetDistrictDecodeByLabel(criteria.DistrictId, Constanst.UseMultiDateTemplate);
        ...
        {
            List<ListItem> bankList = new List<ListItem>();
            var bankIds = _parameters.VirtualTestService.GetDataLockerBank(banks.Select(x => x.Id).ToList());
            bankList = banks.Where(x => bankIds.Contains(x.Id)).ToList();
            return bankList;
        }

        public ActionResult LoadBankByMultipleSubjectIds(LoadBankByMultipleSubjectIdsCriteria criteria)
        {
            if (!CurrentUser.IsPublisher() && !CurrentUser.IsNetworkAdmin)
                criteria.DistrictId = CurrentUser.DistrictId ?? 0;
            criteria.UserId = CurrentUser.Id;
            criteria.UserRole = CurrentUser.RoleId;
            ...
            var banks = _parameters.UserBankService.GetFormBanksByMultipleSubjectIds(criteria).ToList();
            var dataLockerBanks = CheckDataLockerBank(banks);
            var bankOrders = _parameters.BankServices.GetBankOrders(districtId).ToList();
        ...
        {
            if (Session["EntryResultModel"] == null)
                return RedirectToAction("PreviewForm");

            var model = (EntryResultModel)Session["EntryResultModel"];

            if (!model.VirtualTestId.HasValue)
                return RedirectToAction(" PreviewForm");
            ...
            try
            {
                ...
                int districtFilter = model != null && model.DistrictId.HasValue && model.DistrictId > 0 ? model.DistrictId.Value : CurrentUser.DistrictId.Value;
                int schoolId = model != null && model.SchoolId.HasValue && model.SchoolId > 0
